...
describe('MoviesService', () => {
  ...
  describe('getRecentUpcomingMoviesForProfile', () => {
    it('should fetch and combine recent and upcoming movies', async () => {
      ...
      const result = await service.getRecentUpcomingMoviesForProfile('123');
    ...
    it('should handle errors properly', async () => {
      ...
      await expect(service.getRecentUpcomingMoviesForProfile('123')).rejects.toThrow('Database error');
  ...
  describe('addMovieToFavorites', () => {
    it('should add existing movie to favorites', async () => {
      ...
      mockCache.getOrSet.mockResolvedValue(mockRecentUpcoming);
      ...
      expect(mockCache.invalidatePattern).toHaveBeenCalledWith('profile_123_movie');
    ...
    it('should fetch and add new movie from TMDB', async () => {
      ...
      expect(Movie).toHaveBeenCalledWith(
        12345, 'New Movie', 'Description', '2023-01-01', 120, 
        '/poster.jpg', '/backdrop.jpg', 8.5, 'PG-13', 
        undefined, [8, 9], [28, 12]
      );
    ...
    it('should throw error when movie save fails', async () => {
      ...
      (getUSMPARating as jest.Mock).mockReturnValue('PG-13');
      (getUSWatchProviders as jest.Mock).mockReturnValue([8, 9]);
      (errorService.handleError as jest.Mock).mockImplementation((err) => { throw err; });

      await expect(service.addMovieToFavorites('123', 12345)).rejects.toThrow('Failed to save movie information');
      expect(mockMovieInstance.save).toHaveBeenCalled();
    });
  ...
  describe('removeMovieFromFavorites', () => {
    ...
    it('should throw NotFoundError when movie does not exist', async () => {
      ...
      await expect(service.removeMovieFromFavorites('123', 999)).rejects.toThrow('Movie with ID 999 not found');
  ...
  describe('updateMovieWatchStatus', () => {
    it('should update movie watch status successfully', async () => {
      ...
      const result = await service.updateMovieWatchStatus('123', 5, 'WATCHED');
    ...
    it('should throw BadRequestError when update fails', async () => {
      ...
      await expect(service.updateMovieWatchStatus('123', 5, 'WATCHED')).rejects.toThrow(
    ...
    it('should handle database errors', async () => {
      ...
      await expect(service.updateMovieWatchStatus('123', 5, 'WATCHED')).rejects.toThrow('Database error');
